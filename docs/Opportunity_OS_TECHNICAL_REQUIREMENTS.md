...
1. Live-verify the Circle wiring (ST-3) once sandbox credentials + a funded Base Sepolia wallet exist — `scripts/verify-circle-provider.ts` is ready; only the webhook signature crypto has been checked so far, not a real transfer.
2. ST-13/WF-3 remainder: recipient-level webhook reconciliation (both providers), durable milestone-*evidence* waits, and `executeRefund`'s parallel pending-refund gap.
3. UI-1/2/3 polish: sharing permissions, agent steering, richer mission/transaction timelines.
...
7. AZ-1 Supabase JWT production auth; telegram/chain webhooks verified against their own real schemes (Stripe/Circle are now done); API-9 generated client.
