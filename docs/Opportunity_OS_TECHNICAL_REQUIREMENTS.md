...
**Version:** 1.4 (build-reflecting)
**Status:** Living. Phases 0–3 satisfied (Phase 3's fiat rail is now real Stripe test-mode, not just simulated; stablecoin/chain remain local references); Phases 4–5 partial.
**Date:** 2026-09-03
...
- **ST-13** Persisted optimistic/deadman windows (migration 0012, `POST /settlement/plans/:planId/milestones`) + durable Settlement Milestone Timer Workflow (`worker-temporal`: `settlementMilestoneTimerWorkflow` + `checkMilestoneTimerActivity`) that sleeps to the next window and auto-executes `auto_refund`/`auto_release` (including ST-12 recipient splits) exactly like the manual HTTP paths; a disputed plan or an above-threshold optimistic release is reported "held" rather than acted on. **[PARTIAL]** verified via `scripts/verify-settlement-timer-workflow.ts` against the time-skipping Temporal test server, and wired into live traffic (see WF-3). Idempotent execute for Stripe is now **[DONE]** (ST-2, above) and the timer activity shares the same idempotency-safe path. Async provider status reconciliation is now **[PARTIAL]**: `POST /webhooks/stripe` verifies Stripe's real signature scheme over the raw request body (`Stripe.webhooks.constructEvent`, `main.ts`'s `rawBody: true`) and reconciles `payment_intent.succeeded`/`charge.refunded` (idempotent — releases/refunds a milestone found `pending`, no-ops if already settled) and `payment_intent.payment_failed`/`charge.dispute.created` (disputes the milestone, conservative — a human resolves it) by correlating the event's PaymentIntent id to `settlement_milestones.provider_ref`. Not yet reconciled: recipient-level Transfer events (`transfer.reversed`, ST-12) — those live in `recipients_json`, not a plain column, and need a different lookup.
...
- **API-7** Webhooks: stripe/telegram/chain. **[PARTIAL]** Stripe now verifies the provider's actual signature scheme over the raw body and reconciles settlement state (see ST-13); telegram/chain still verify against a re-serialization of the parsed body via a generic HMAC helper, not each provider's real scheme.
...
- **Phase 3** Native Money Rails — exit: a release moves funds on a selected rail behind the escrow engine + evidence ledger + release policy — **MET, including a real provider (ST-1..ST-14, UI-4, WF-1..WF-3):** the fiat rail runs real Stripe test-mode API calls (not just simulation) when `STRIPE_SECRET_KEY` is set, with idempotent execute and webhook-verified reconciliation; open: recipient-level (Transfer) webhook reconciliation, durable milestone-evidence waits, and — a business/compliance decision, not a code change — an actual licensed money-transmitter partnership + audit for going live with real funds (§C-6).
...
1. ST-13/WF-3 remainder: recipient-level (ST-12 Transfer) webhook reconciliation, and durable milestone-*evidence* waits.
2. UI-1/2/3 polish: sharing permissions, agent steering, richer mission/transaction timelines (UI-4 is now done).
...
6. AZ-1 Supabase JWT production auth; telegram/chain webhooks verified against their own real schemes (Stripe is now done); API-9 generated client.
